...
def ige_decrypt(cipher_text: bytes, key: bytes, iv: bytes):
    iv_part1 = iv[:16]
    iv_part2 = iv[16:]
    cipher_text += bytes(-len(cipher_text) % 16)

    cipherer = Cipher(algorithms.AES(key), modes.ECB())

    text = b''
    ciphers = zip(*[iter(cipher_text)] * 16)
    for cipher in ciphers:
        decryptor = cipherer.decryptor()
        part1 = bytes(a ^ b for a, b in zip(cipher, iv_part2))
        part2 = decryptor.update(part1) + decryptor.finalize()
        plain = bytes(a ^ b for a, b in zip(part2, iv_part1))
